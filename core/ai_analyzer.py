...
규칙:
- 섹션 제목(분석 분류, 핵심 요약, 실행 흐름, 행위 분석, 결론)은 절대 변경하지 마세요.
- 필드 이름(위협 수준:, 주요 분석 대상:, 설명:, 태그 및 해석: 등)도 절대 변경하지 마세요.
- 대괄호 [ ] 안 내용만 실제 분석 내용으로 교체하세요.
- 마크다운(##, **, -, ``` 등) 사용 금지. 순수 텍스트로만 출력하세요.
- 대응 권고는 절대 포함하지 마세요.
- 추측은 "추정" 표현을 사용하고, 확인된 사실과 구분하세요.
구체성 요건 (반드시 준수):
- 각 항목에서 실제 관찰된 파일명, 전체 경로, IP 주소, 도메인, 레지스트리 경로, 프로세스명을 직접 인용하세요.
- "다수의 IP", "시스템 도구", "악성 서버", "특정 경로" 같은 추상 표현은 절대 금지합니다.
- 행위 분석 각 항목에서 해당 행위가 없으면 반드시 "관찰되지 않음"으로 명시하세요.
- 태그 및 해석의 각 태그는 탐지 근거를 완전한 한 문장으로 기술하세요.
...
주요 분석 대상: {sample_name} ([실행 맥락 — 실행 디렉터리와 실행 방식을 구체적으로 기술, 예: 사용자 Temp 디렉터리에서 직접 실행])
설명: [한 문장 — 악성코드 패밀리(추정) 또는 유형, 핵심 기능, C2/유출 프로토콜을 구체적으로 포함]
태그 및 해석:
[탐지된 태그 이름]: [이 태그가 탐지된 구체적 근거를 완전한 한 문장으로 기술 — 파일명·도메인·기술 ID 등 인용]
[탐지된 태그 이름]: [이 태그가 탐지된 구체적 근거를 완전한 한 문장으로 기술]

핵심 요약
[2~3문장. 악성코드 패밀리(추정)와 실행 진입점, 핵심 악성 행위(드롭, C2, 유출 등)를 구체적 파일명·IP·도메인과 함께 서술. 분석 데이터에서 확인된 사실만 기술.]

실행 흐름
[사용자 행위] [사용자가 직접 유발한 단계 — 구체적 파일명과 실행 위치 포함]
[준비 단계] [악성코드가 수행한 스테이징/환경 준비 — 드롭한 파일명과 전체 경로 포함]
[자율 실행] [자동화된 악성 행위 — C2 도메인 또는 IP, 프로토콜, 유출 크기 등 포함]

행위 분석
로더 / 스테이징: [{sample_name}의 역할과 드롭하거나 메모리에 로드한 페이로드의 파일명·경로를 직접 인용]
실행 및 피벗 (LOLBin / 인터프리터): [사용된 시스템 도구 이름(예: tasklist.exe, cmd.exe, powershell.exe)과 각 도구의 목적. 해당 없으면 "관찰되지 않음"]
지속성 (관찰된 경우): [레지스트리 전체 키 경로, 서비스명, 스케줄 작업 이름을 직접 인용. 해당 없으면 "관찰되지 않음"]
탐색 / 수집 (관찰된 경우): [수집된 정보 유형(자격증명, 시스템 정보, 스크린샷 등)과 사용한 프로세스명 인용. 해당 없으면 "관찰되지 않음"]
네트워크 / C2 또는 유출 (관찰된 경우): [C2 도메인 또는 IP, 사용 프로토콜(FTP/HTTP/SMTP 등), 전송 크기 또는 업로드 파일명을 직접 인용. 해당 없으면 "활동 없음"]
오류 / 크래시 (관찰된 경우): [실행 중 관찰된 오류, 비정상 종료, 크래시 정보. 해당 없으면 "관찰되지 않음"]

결론
[1~2문장. 최종 위협 판단과 공격자 의도를 분석 데이터의 구체적 증거와 함께 서술.]"""
...
def _call_ollama(
    ...
    payload = json.dumps({
        ...
        "options": {
            "temperature": 0.2,
            "num_ctx":     8192,
            "num_predict": 2048,   # 완전한 보고서 출력에 필요한 최소 길이
        },
    }).encode("utf-8")
